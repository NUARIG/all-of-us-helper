  ...
  task(compare_healthpro_to_study_tracker: :environment) do  |t, args|
    ...
    # batch_health_pro.health_pros.where("pmi_id in (?) AND biospecimens_location = ? AND general_consent_status = '1' AND general_consent_date IS NOT NULL AND ehr_consent_status = '1' AND ehr_consent_date IS NOT NULL AND withdrawal_status = '0' AND withdrawal_date IS NULL", 'nwfeinberggalter').each do |health_pro|
      ...
      study_tracker_activities_by_case_number.each do |study_tracker_activity|
        ...
        puts 'state'
        puts study_tracker_activity.to_hash['state']
      end

      study_tracker_activity_consented = study_tracker_activities_by_case_number.select { |study_tracker_activity| study_tracker_activity.to_hash['name'] == 'Consented' && study_tracker_activity.to_hash['date'].present? &&  study_tracker_activity.to_hash['state'] == 'completed' }.max_by { |study_tracker_activity| Date.parse(study_tracker_activity.to_hash['date']) }
      ...
      study_tracker_activity_ehr_consent = study_tracker_activities_by_case_number.select { |study_tracker_activity| study_tracker_activity.to_hash['name'] == 'EHR Consent' && study_tracker_activity.to_hash['date'].present? &&  study_tracker_activity.to_hash['state'] == 'completed' }.max_by { |study_tracker_activity| Date.parse(study_tracker_activity.to_hash['date']) }
      ...
      study_tracker_activity_withdrawal = study_tracker_activities_by_case_number.select { |study_tracker_activity| study_tracker_activity.to_hash['name'] == 'Withdrawn' && study_tracker_activity.to_hash['date'].present? &&  study_tracker_activity.to_hash['state'] == 'completed' }.first
    ...
    case_numbers.each do |case_number|
      ...
      study_tracker_activity_consented = case_number_study_tracker_activities_by_case_number.select { |study_tracker_activity| study_tracker_activity.to_hash['name'] == 'Consented' && study_tracker_activity.to_hash['date'].present? &&  study_tracker_activity.to_hash['state'] == 'completed' }.first
      if study_tracker_activity_consented
        subject[:nmhc_mrn] = study_tracker_activity_consented.to_hash['nmhc_record_number']
        subject[:general_consent_status_st] = '1'
      ...
      study_tracker_activity_ehr_consent = case_number_study_tracker_activities_by_case_number.select { |study_tracker_activity| study_tracker_activity.to_hash['name'] == 'EHR Consent' && study_tracker_activity.to_hash['date'].present? &&  study_tracker_activity.to_hash['state'] == 'completed' }.first
      if study_tracker_activity_ehr_consent
        subject[:ehr_consent_status_st] = '1'
      ...
      study_tracker_activity_withdrawal = case_number_study_tracker_activities_by_case_number.select { |study_tracker_activity| study_tracker_activity.to_hash['name'] == 'Withdrawn' && study_tracker_activity.to_hash['date'].present? &&  study_tracker_activity.to_hash['state'] == 'completed' }.first
      if study_tracker_activity_withdrawal
        subject[:withdrawal_status_st] = '1'
        subject[:withdrawal_date_st] = Date.parse(study_tracker_activity_withdrawal.to_hash['date']) if study_tracker_activity_withdrawal.to_hash['date']
      else
