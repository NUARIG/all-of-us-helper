class DeclinedPatientsController < ApplicationController
  ...
  def index
    ...
    @declined_patients = HealthPro.declined.by_biospecimens_location_declined(params[:biospecimens_location]).search_across_fields_declined(params[:search], options).paginate(per_page: 10, page: params[:page])
  end
